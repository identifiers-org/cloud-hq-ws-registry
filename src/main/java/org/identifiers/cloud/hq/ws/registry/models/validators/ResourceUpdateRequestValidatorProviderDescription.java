...
public class ResourceUpdateRequestValidatorProviderDescription implements ResourceUpdateRequestValidator {
    // TODO
    @Autowired
    ...
    public boolean validate(ServiceRequestUpdateResourcePayload request) throws ResourceUpdateRequestValidatorException {
        try {
            return (request.getProviderDescription() == null)
                    || (delegateValidator.validate(ApiAndDataModelsHelper.getFrom(request)));
        } catch (PrefixRegistrationRequestValidatorException e) {
            throw new ResourceUpdateRequestValidatorException(String.format("INVALID Provider Description '%s', '%s'", request.getProviderDescription(), e.getMessage()));
        }
    }
}
